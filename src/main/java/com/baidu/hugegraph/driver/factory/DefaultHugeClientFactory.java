...
public class DefaultHugeClientFactory {
    ...
    private static final String CLIENT_KEY_PATTERN = "%s-%s-%s";
    ...
    public HugeClient createClient(String graphSpace, String graph,
        ...
        String key = Strings.lenientFormat(CLIENT_KEY_PATTERN, token,
                                           username, password);
        ...
        if (client == null) {
            synchronized (CLIENT_CACHES) {
                ...
                if (client == null) {
                    client = HugeClient.builder(this.urls[r], graphSpace, graph)
                                       .configToken(token)
